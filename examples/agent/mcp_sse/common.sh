...
GATEWAY_URL="${GATEWAY_URL:-${ALEPHANT_GATEWAY_URL:-${AI_GATEWAY_BASE_URL:-http://127.0.0.1:8080}}}"
API_KEY="${ALEPHANT_API_KEY:-${API_KEY:-${ALEPHANT_CONTROL_OPENROUTER_API_KEY:-${OPENAI_API_KEY:-}}}}"
AGENT_ID="${AGENT_ID:-${ALEPHANT_AGENT_ID:-mcp-sse-demo-agent}}"
AGENT_NAME="${AGENT_NAME:-${ALEPHANT_AGENT_NAME:-MCP SSE Demo Agent}}"
RUN_ID="${RUN_ID:-${ALEPHANT_RUN_ID:-run_mcp_sse_$(random_suffix)}}"
