"""CrewAI-compatible OpenAPI Agent Tool demo."""
...
def raw_adapter_demo(client: OpenApiToolClient) -> dict[str, Any]:
    ...
    schema_invalid = client.call_tool(
        ...
        step_id="step_crewai_schema_invalid",
    )
    return {
...
def crewai_adapter_demo(client: OpenApiToolClient) -> dict[str, Any]:
    try:
    ...
    class GatewayOpenApiTool(BaseTool):
        name: str = descriptor.get("frameworkToolName", "support_get_ticket")
        description: str = descriptor.get("description") or "Gateway OpenAPI tool"

        def _run(self, ticket_id: str = "T-1001") -> str:
            response = client.call_with_refresh_once(
                tool_id=tool_id,
    ...
    return {
        "framework": "crewai",
        "sdk_installed": True,
        "registered_tool_name": tool.name,
        "tool_id": tool_id,
        "tool_output": tool._run(os.getenv("OPENAPI_TICKET_ID", "T-1001")),
        "schema_invalid": envelope_to_text(schema_invalid),
    }


def main() -> None:
    print_json(crewai_adapter_demo(OpenApiToolClient()))
