...
impl App {
    ...
    async fn build_app_state(config: Config) -> Result<AppState, InitError> {
        if Self::policy_grpc_required(&config) && config.policy.grpc_endpoint.trim().is_empty() {
        ...
        let x402_payment =
            crate::x402::payment_client::X402PaymentClientHolder::new(x402_payment_initial);
...
pub async fn build_test_app(mut config: Config) -> Result<App, InitError> {
    ...
    let app_state = AppState(Arc::new(InnerAppState {
        ...
        x402_payment: crate::x402::payment_client::X402PaymentClientHolder::new(None),
        workspace_provider_allowlist: std::sync::RwLock::new(rustc_hash::FxHashMap::default()),
        provider_is_router_flags: std::sync::RwLock::new(rustc_hash::FxHashMap::default()),
        llm_kv,
        semantic_cache: None,
...
#[cfg(any(test, feature = "testing"))]
pub async fn build_test_gateway_app(config: Config) -> Result<App, InitError> {
